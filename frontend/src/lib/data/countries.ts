/**
 * ISO 3166-1 countries with their E.164 calling codes.
 *
 * The phone field used to be a bare text box with a "+1 555 000 0000" hint, so
 * a guest outside North America had to know to type their own country code and
 * most did not. Twilio is handed `To` verbatim and rejects anything that is not
 * E.164, so those numbers simply never received a code.
 *
 * Flags are derived from the ISO code rather than stored: a regional-indicator
 * pair IS the flag emoji, so there is no second list to drift out of sync.
 */
...
/** The markets CatoDrive operates in today, floated to the top of the picker. */
export const PRIORITY_CODES = ['US', 'IN', 'CA', 'GB'] as const;

const BY_CODE = new Map(COUNTRIES.map((c) => [c.code, c]));
export const countryByCode = (code: string): Country | undefined => BY_CODE.get(code.toUpperCase());

/**
 * Countries ordered for a picker: the operating markets first, then the rest
 * alphabetically. Sorted with a locale collator so accented names (Åland,
 * Côte d'Ivoire) land where a reader expects rather than after Z.
 */
export const PICKER_ORDER: Country[] = (() => {
  const priority = PRIORITY_CODES.map((c) => BY_CODE.get(c)).filter((c): c is Country => !!c);
  const rest = COUNTRIES.filter((c) => !PRIORITY_CODES.includes(c.code as (typeof PRIORITY_CODES)[number])).sort(
